...
class GamblerPlayer(Player):
    ...
    def __init__(self, name: str, model: ModelInterface, assets: float = 100, persona: str = ""):
        ...
        # Study state
        self.wage_multiplier = 1.0  # starts at 1x, doubles each completed study
    ...
    def advance_study(self) -> dict:
        ...
        if self.studying_remaining <= 0:
            self.wage_multiplier *= 2.0
            self.study_count += 1
            return {"event": "study_complete", "new_multiplier": self.wage_multiplier}
    ...
    def effective_wage(self) -> float:
        return self.wage_multiplier  # base wage is multiplied in engine
